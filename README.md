...
The good news: two controls block the majority of these attacks regardless of ecosystem. This post covers the exact config for npm, pnpm, yarn, bun, pip, uv, Go, Maven, Gradle, NuGet, and Cargo — plus a one-command proxy setup that enforces these controls org-wide, and shell tests you can run today to verify your config actually works.
...
| Java | Maven | `pom.xml` + `settings.xml` | none (use private proxy) | no install scripts by design |
| Java | Gradle | `build.gradle.kts` + `gradle.properties` | none (use private proxy) | no install scripts by design |
| .NET | NuGet | `nuget.config` + `*.csproj` | none (use private proxy) | no install scripts; IL weaving is the threat |
